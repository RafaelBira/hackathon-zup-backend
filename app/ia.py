...
def recommend_articles(user_id):
    ...
    contents_str = ", ".join(user_interests)  # Convertendo interesses em uma string
    articles_str = ". ".join(all_articles)  # Convertendo os artigos em uma string
    ...
    api_request_json = {
        "messages": [
            {
                "role": "system",
                "content": f"Interesses do usuário: {contents_str}. Artigos: {articles_str}",
            },
            {
                "role": "user",
                "content": "Responda separado por Artigo em ligua portuguesa as recomendacoes que se enquadram no perfil desse usuario, apenas o nome do artigo e nada mais. Resposta deve ser em portugues separando os resultados por ';'",
            },
        ],
        "max_tokens": 200,
    }
    ...
    content_total = response.json()  # Converte a resposta para um dicionário
    recommended_articles = content_total["choices"][0]["message"][
        "content"
    ]  # Acessa o campo específico

    return recommended_articles  # Retorna os artigos recomendados
